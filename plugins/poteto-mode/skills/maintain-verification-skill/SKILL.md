...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map, usually `.codex/skills/verify-*/`. If several candidates exist and context does not identify one, ask. If none exists, stop and point at `create-verification-skill` instead of inventing a target.
